...
one.bucket <- list()
stock.names <- "stock_0.csv"

for (stock_name in stock.names) {
  stock.data <- read.csv(paste(DIRECTORYNAME, "/", stock_name, sep=''))
  buckets <- list()
  bucket_ids <- unique(stock.data$time_id)
  for (bucket_id in bucket_ids) {
    ...
    bucket.inter <- bucket.data %>% 
      ...
      na.locf()
   buckets <- append(buckets, bucket.inter)
   break
  }
  one.bucket <- append(one.bucket, buckets)
  break
}
one.bucket <- as.data.frame(one.bucket)
one.bucket
```
