...
#Given a bucket, perform svr prediction.
```{r}
...
ggplot(data = cleaned.svm.df) + 
      geom_boxplot(mapping = aes(x = Volatility, y = rmse)) + 
      theme_minimal() +
      ggtitle("RMSE Accuracy using SVR, buckets/bin = 5, repeat = 30") + 
      theme(axis.text.x = element_text(angle = 45, hjust=1))
...
ggplot(data = cleaned.svm.df) + 
      geom_boxplot(mapping = aes(x = Volatility, y = qlike)) + 
      theme_minimal() +
      ggtitle("QLIKE Accuracy using SVR, buckets/bin = 5, repeat = 30") + 
      theme(axis.text.x = element_text(angle = 45, hjust=1))
...
ggplot(data = cleaned.svm.df) + 
      geom_boxplot(mapping = aes(x = Volatility, y = mae)) + 
      theme_minimal() +
      ggtitle("MAE Accuracy using SVR, buckets/bin = 5, repeat = 30") + 
      theme(axis.text.x = element_text(angle = 45, hjust=1))
...
aRMSE = ggplot() +
  ...
  scale_color_manual(labels = c("ARM", "SVR", "REG", "RAN"), values = c("red", "darkgreen", "blue", "purple")) + 
...
```{r}
my_save1 = list(aRMSE = aRMSE, aQLIKE = aQLIKE, aMAE = aMAE, mRMSE = mRMSE, mQLIKE = mQLIKE, mMAE = mMAE)
saveRDS(my_save1,"my_save1.rds")
```
...
Absolute error is the absolute difference between the predicted and the actual i.e., actual: -1, predicted: 1, abs_error = 2
Mean_error is obsolete cuz its not as good as describing accuracy as absolute
```{r}
...
This randomly selects buckets given the number of buckets,
calculates the actual trend,
predicts the trend like a streaming classifier using three models
Then outputs a table of accuracies
Warning: it takes an hour to run
```{r, warning=FALSE}
set.seed(3888)
...
This shows the distribution of fluctuations.
We picked 20000 buckets, and each bucket has a fluctuation
Fluctuation is defined as the number of trend changes, noted as the second derivative of price
So, the vector 0 1 1 -1 will have a fluctuation of 2 because the numbers have changed twice.
We can see that most buckets hav ea fluctuation betwen 10 and 12.
```{r}
#Ignore this one
...
#Sbows the heatmap for knn
So this shows how well the model does in a specific fluctuation level and how much aboslute error it gains.
There is a trend shown by the line of best fit.
threshold specifies the number of buckets in each fluctuation to simulate a uniform distirbution because the one before is a bell curve and since we want to be fair we use unifrom
```{r, warning=FALSE}
...
This is the graph with all the line of best fit from ran, svm, knn all together
Can compare to see which one performs better on average on different fluctuations
```{r, warning=FALSE}
...
This is for the shiny app, and basically the final product
- Predicts the next 30 seconds based on last 480 (must input 600 seconds of data to predict the next 30 seconds (630))
- PRedicts the trend based on what fluctuation it is, if it is less than 11 we use knn and if it is more we use ran
- Predicts volatility, we found in inno1 ARM was better so we use that
- Predicts the accuracy of the trend prediction using the line of best fit.
```{R}
...
ui <- fluidPage(theme = shinytheme("cyborg"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(inputId = "model_2", label = "Choose a time_id (among 5, 11, 16, 31, 62)",
