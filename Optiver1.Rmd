...
one.bucket <- calculate_using_mean_time_bucket(one.bucket, 30)
```
...
QLIKE <- function(predictied, actual) {
  return (mean(2 * (actual * log(actual/predictied) - (actual - predictied))))
}
```
...
MSE(result$predicted_volatility, result$actual_volatility)
QLIKE(result$predicted_volatility, result$actual_volatility)
...
result <- regression_model(one.bucket)
result
MSE(result$predicted_volatility, result$actual_volatility)
QLIKE(result$predicted_volatility, result$actual_volatility)
mae(result$actual_volatility, result$predicted_volatility)
...
ARMA_GARCH_model <- function(bucket) {
  ...
  future.path <- fitted(ugarchpath(spec, n.sim = 60, m.sim = 1000))
  future.path[is.na(future.path)] <- 0
  predicted <- append(predicted, mean(sqrt(colSums(future.path ^ 2))))
  
  future.path <- fitted(ugarchpath(spec, n.sim = 90, m.sim = 1000))
  future.path[is.na(future.path)] <- 0
  predicted <- append(predicted, mean(sqrt(colSums(future.path ^ 2))))
  
  future.path <- fitted(ugarchpath(spec, n.sim = 120, m.sim = 1000))
  future.path[is.na(future.path)] <- 0
  predicted <- append(predicted, mean(sqrt(colSums(future.path ^ 2))))
  
  mean_predict <- mean(predicted)
  mean_test.Y <- mean(calculate_using_mean_time_bucket(bucket, 30)[17:20, "volatility"])
  print(mean_predict/mean_test.Y)
}
ARMA_GARCH_model(one.bucket)
```
